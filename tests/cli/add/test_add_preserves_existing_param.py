...
from terrifying.policies.library import load_manifest, filter_by_engine, PolicyEntry, ParamDescriptor
from terrifying.policies.add import run_add
import yaml


def _entry_with_param():
    base = [e for e in filter_by_engine(load_manifest(), "rego") if e.id == "rds-storage-encrypted"][0]
    import dataclasses
    return dataclasses.replace(base, params=[ParamDescriptor("required_tags", "list[string]", "Required tag keys", ["Environment"])])

...
POLICY = Path(__file__).parent.parent.parent.parent.parent.parent / "terrifying/policies/library/networkfirewall/netfw-logging-enabled.yml"
...
def test_compliant():
    ...
    tf = tf_resource(
        "aws_networkfirewall_logging_configuration", "logs",
...
def test_violation():
    tf = tf_resource(
        "aws_networkfirewall_firewall", "fw",
        '  name   = "example"\n  vpc_id = "vpc-12345"\n',
    )
    assert c7n_violations(POLICY, tf) != []

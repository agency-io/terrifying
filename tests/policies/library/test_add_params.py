...
def test_collect_params_prompts_for_new_params(tmp_path):
    entry = _make_entry("p1", "rego", [ParamDescriptor(name="min_days", type="str", description="min days", default=7)])
...
def test_collect_params_shared_params_both_engines(capsys):
    rego = _make_entry("p1", "rego", [ParamDescriptor(name="tags", type="str", description="required tags", default=None)])
    c7n = _make_entry("p2", "c7n", [ParamDescriptor(name="tags", type="str", description="required tags", default=None)])
...
def test_collect_params_skips_existing():
    entry = _make_entry("p1", "rego", [ParamDescriptor(name="min_days", type="str", description="x", default=7)])
...
def test_collect_params_eofError_uses_default():
    entry = _make_entry("p1", "rego", [ParamDescriptor(name="x", type="str", description="x", default="fallback")])
...
def test_build_delta_with_opa_params(tmp_path):
    ...
    from terrifying.policies.library import load_manifest, filter_by_engine
    entries = [e for e in filter_by_engine(load_manifest(), "rego") if e.id == "rds-storage-encrypted"]
    param = ParamDescriptor(name="test_param", type="str", description="x", default="val")
    ...
    assert delta.new_config["policies"]["opa"].get("params", {}).get("test_param") == "myval"
...
def test_build_delta_c7n_with_params(tmp_path):
    ...
    if not entries:
        return
    param = ParamDescriptor(name="c7n_param", type="str", description="x", default="default_val")
    patched = dataclasses.replace(entries[0], params=[param])
    with patch("builtins.input", return_value="newval"):
        result = _collect_params([patched], {})
    delta = _build_delta([patched], result, tmp_path / "terrifying.yml")
    assert "c7n" in delta.new_config["policies"]
    assert delta.new_config["policies"]["c7n"].get("params", {}).get("c7n_param") == "newval"

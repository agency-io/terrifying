...
def test_compliant_python311():
    inp = rego_input(
        [resource("aws_lambda_function", "my_fn", {"runtime": "python3.11"})]
    )
...
def test_compliant_nodejs20():
    inp = rego_input(
        [resource("aws_lambda_function", "my_fn", {"runtime": "nodejs20.x"})]
    )
    assert eval_rego_policy(POLICY, inp) == []
...
def test_violation_python27():
    inp = rego_input(
        [resource("aws_lambda_function", "my_fn", {"runtime": "python2.7"})]
    )
    assert eval_rego_policy(POLICY, inp) != []
...
def test_violation_nodejs12():
    inp = rego_input(
        [resource("aws_lambda_function", "my_fn", {"runtime": "nodejs12.x"})]
    )
    assert eval_rego_policy(POLICY, inp) != []
...
def test_violation_dotnetcore21():
    inp = rego_input(
        [resource("aws_lambda_function", "my_fn", {"runtime": "dotnetcore2.1"})]
    )
    assert eval_rego_policy(POLICY, inp) != []

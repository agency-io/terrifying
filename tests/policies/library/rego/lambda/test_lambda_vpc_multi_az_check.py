...
POLICY = Path(__file__).parent.parent.parent.parent.parent.parent / "terrifying/policies/library/lambda/lambda-vpc-multi-az-check.rego"


def test_compliant_two_subnets():
    inp = rego_input([resource("aws_lambda_function", "my_fn", {
        "vpc_config": [{"subnet_ids": ["subnet-aaa", "subnet-bbb"], "security_group_ids": ["sg-aaa"]}],
    })])
...
def test_compliant_three_subnets():
    inp = rego_input([resource("aws_lambda_function", "my_fn", {
        "vpc_config": [{"subnet_ids": ["subnet-aaa", "subnet-bbb", "subnet-ccc"], "security_group_ids": ["sg-aaa"]}],
    })])
    assert eval_rego_policy(POLICY, inp) == []
...
def test_violation_one_subnet():
    inp = rego_input([resource("aws_lambda_function", "my_fn", {
        "vpc_config": [{"subnet_ids": ["subnet-aaa"], "security_group_ids": ["sg-aaa"]}],
    })])
    assert eval_rego_policy(POLICY, inp) != []

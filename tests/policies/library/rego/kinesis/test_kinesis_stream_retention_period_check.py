...
def test_compliant_168_hours():
    inp = rego_input(
        [resource("aws_kinesis_stream", "my_stream", {"retention_period": 168})]
    )
...
def test_compliant_more_than_168():
    inp = rego_input(
        [resource("aws_kinesis_stream", "my_stream", {"retention_period": 336})]
    )
    assert eval_rego_policy(POLICY, inp) == []
...
def test_violation_24_hours():
    inp = rego_input(
        [resource("aws_kinesis_stream", "my_stream", {"retention_period": 24})]
    )
    assert eval_rego_policy(POLICY, inp) != []
...
def test_violation_167_hours():
    inp = rego_input(
        [resource("aws_kinesis_stream", "my_stream", {"retention_period": 167})]
    )
    assert eval_rego_policy(POLICY, inp) != []

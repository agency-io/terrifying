...
# description: Detects S3 buckets with public read access via ACL grants to AllUsers or AuthenticatedUsers. Equivalent to AWS Config s3-bucket-public-read-prohibited.
# severity: Critical
...
deny contains msg if {
    ...
    msg := sprintf("Resource %v.%v: S3 bucket does not block public read access", [bucket.type, bucket.name])
}
...
_has_public_access_block(resources, bucket_name) if {
    ...
    r.attributes.ignore_public_acls == true
}

...
- [x] 1.1 Create `terrifying/rules/best_practices/` package with `__init__.py`
- [x] 1.2 Implement `NoHardcodedValues(allowed_attributes: list[str] | None = None)` in `no_hardcoded_values.py`
...
- [x] 1.3 Implement `VariablesHaveDescriptions` in `variables_have_descriptions.py`
- [x] 1.4 Implement `OutputsHaveDescriptions` in `outputs_have_descriptions.py`
- [x] 1.5 Implement `RequiredTags(tags: list[str])` in `required_tags.py`
...
- [x] 1.6 Verify each rule file does not exceed 500 lines

## 2. Tests (`tests/rules/best_practices/`)

- [x] 2.1 `test_no_hardcoded_values.py`:
...
- [x] 2.2 `test_variables_have_descriptions.py`:
...
- [x] 2.3 `test_outputs_have_descriptions.py`:

...
- [x] 1.1 Create `terrifying/rules/structural/` package with `__init__.py`
- [x] 1.2 Implement `MaxResourcesPerFile(max_resources: int = 10)` in `max_resources_per_file.py` — one class, one file
- [x] 1.3 Implement `MaxLinesPerFile(max_lines: int = 150)` in `max_lines_per_file.py`
- [x] 1.4 Implement `ResourceFileNaming(pattern: str)` in `resource_file_naming.py` — compiles regex in `__init__`
- [x] 1.5 Verify each rule file does not exceed 500 lines (all will be well under; noted for consistency)

## 2. Tests (`tests/rules/structural/`)

- [x] 2.1 `test_max_resources_per_file.py`:
...
- [x] 2.2 `test_max_lines_per_file.py`:
  - Below limit → no violations
  - Exactly at limit → no violations (boundary)
...
- [x] 2.3 `test_resource_file_naming.py`:
